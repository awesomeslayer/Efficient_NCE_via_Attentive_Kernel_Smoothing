...
class NeuralCDE(BaseModel):
    ...
    def make_interpolation(self, coeffs):
        it, kp, at = self.params['interp'], self.params['kp'], self.params['at']
        ...
        if it == 'kernel': return KernelInterpolation(coeffs, t, kp, at)
        if it == 'gp': return GPInterpolation(coeffs, t, kp, at)
        return None
    ...
    def forward(self, coeffs):
        ...
        z_T = torchcde.cdeint(X=X, func=self.func, z0=z0, t=X.interval, atol=self.params['tol'], rtol=self.params['tol'],)#method='bosh3')
...
class MultiHeadCDEBase(BaseModel):
    ...
    def _get_weighted_interp(self, coeffs, weights):
        ...
        b_tensor = torch.tensor(self.bandwidths, device=coeffs.device, dtype=coeffs.dtype).repeat(batch_size)
        
        if self.kernel == 'gp':
            return WeightedGPInterpolation(c_par, w_par, self.t_grid, {'length_scale': b_tensor, 'noise_std': self.noise_std}, self.add_time)
...
class QFormerCDE(MultiHeadCDEBase):
    ...
    def forward(self, coeffs):
        ...
        def make_it():
            ...
            self._last_attn = attn # Оставляем на GPU и в графе вычислений!    
